        ...
        <div class="title">
            <div  class="title-pic">
                <div class="title-content">
                    <div><h1>数据展示</h1></div>
                    <div class="search">
                        <Input v-model="searchName" placeholder="搜索数据名称" style="width: 350px" />
            ...
            <div class="content-plate">
                ...
                <div class="content-pic">
                    <div class="pic" v-for="item in moduleData" :key="item.id">
                        <img class="display-pic" :src="item.datasetPic" alt="">
                        <div>
                            <div class="pic-title">
                                {{item.datasetName}}
                            </div>
                            <div class="jianjie">{{item.datasetDesc}}</div>
                            <div class="button">
                                <Button @click="viewDetails(item.id)">查看详情</Button>
                                <Button v-if="item.joinDatasetCenter == '已加入'" disabled>已申请</Button>
                                <Button v-if="item.joinDatasetCenter == '申请中'">申请中</Button>
                                <Button class="buttonJoin" v-if="item.joinDatasetCenter == '未申请'"  @click="handleApplyJoin(item.id,item.datasetName)">申请加入</Button>
                            </div>
                        </div>
                    </div>
                </div>
...
import { getDataList, applyJoin } from '@/services/api/gateway'
export default {
            ...
            getDataList({
                ...
                .then(response => {
                    ...
                    this.moduleData.forEach((item) => {
                        item.datasetPic = BASE_URL + item.datasetPic
                    })
                })
        },
        nameSearch () {
